...
function translate_trash_list_info_to_html(trash_list_info){
    ...
    for(var i= trash_list_info.length-1; i>=0; i-- ){
        var trash_content_preview = preview_note_content(trash_list_info[i].content);
        result_html += '<li id="' +trash_list_info[i].id + '"><a onclick="confirm_recover_note('+ trash_list_info[i].id +')"><h2>'
            + preview_note_title(trash_list_info[i].title) +
            '</h2><p><strong>' + trash_content_preview + '</strong></p><p class="ui-li-aside">' +
